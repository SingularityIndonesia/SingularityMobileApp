    ...
    override suspend fun <T : Form> insert(form: T): Result<T> {
        return runCatching(Dispatchers.IO) {
            newSuspendedTransaction(Dispatchers.IO) {
                ...
                FormsTable.insert {
                    it[FormsTable.uuid] = form.uuid
                    it[FormsTable.createdAt] = now
                    it[FormsTable.updatedAt] = now
                    it[FormsTable.validUntil] = validUntil
                    it[FormsTable.header] = form.header?.let { header -> json.encodeToString(header) }
                    it[FormsTable.body] = form.body?.let { body -> json.encodeToString(body) }
                }
